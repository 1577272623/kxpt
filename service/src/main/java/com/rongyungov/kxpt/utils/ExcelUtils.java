package com.rongyungov.kxpt.utils;
...
public class ExcelUtils {
     ...
     * @return
     */
    public static List<String[]> readExcel(String filePath, Integer sheetIndex) {
        List<String[]> dataList = new ArrayList<>();
        Workbook wb = ExcelUtils.createWorkBook(filePath);
        if (wb != null) {
            ...
            for (int i = 0; i < maxRownum; i++) {
                String[] map = new String[maxColnum];
                if (i > 0) {
//                    map = new LinkedHashMap<>();
                ...
                if (firstRow != null) {
                    ...
                    for (int j = 0; j < maxColnum; j++) {
                        cellData = (String) ExcelUtils.getCellFormatValue(firstRow.getCell(j));
                        if (i == 0) {
                            columns[j] = cellData;
                        } else {
                            map[j] =cellData;
